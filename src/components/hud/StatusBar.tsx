...
export default function StatusBar() {
  ...
  const gpsSats = status?.gpsSats ?? 0;
  ...
  return (
    <div className="hud-panel p-4 w-full md:w-[280px]">
      ...
      {/* State indicator */}
      <div className="flex items-center gap-2 mb-2" title="Dish connection state reported via gRPC">
      ...
      {/* Uptime */}
      <div className="flex justify-between items-baseline mb-1.5" title="Time since last dish reboot">
      ...
      {/* Connection quality */}
      <div className="flex justify-between items-center mb-1.5" title="Percentage of packets successfully delivered (1 - drop rate)">
      ...
      {/* Device info */}
      <div className="flex justify-between items-baseline mb-1" title="Dish firmware version">
      ...
      <div className="flex justify-between items-baseline mb-1" title="Number of GPS satellites used for dish positioning">
        <span className="text-[10px] text-white/40">GPS</span>
        <span className="text-[10px] text-white/40 tabular-nums">{gpsSats} satellites</span>
      </div>
    </div>
  );
}
