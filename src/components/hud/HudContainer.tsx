...
function MobileStatusPill() {
  ...
  return (
    <div className="hud-panel px-3 py-2 flex items-center gap-2 text-[12px]">
...
export default function HudContainer() {
  ...
  return (
    ...
    >
      {/* HUD toggle — always visible, bright on mobile */}
      <button
        onClick={toggleHud}
        className={`absolute top-4 right-4 md:top-6 md:right-6 pointer-events-auto z-20 flex items-center justify-center rounded transition-colors ${
          isMobile
            ? 'w-11 h-11 bg-black/70 border-2 border-cyan-400/60 text-cyan-400 shadow-[0_0_8px_rgba(0,212,255,0.3)]'
            : 'w-8 h-8 bg-black/40 border border-white/10 hover:border-white/30 text-white/50 hover:text-white/80'
        }`}
